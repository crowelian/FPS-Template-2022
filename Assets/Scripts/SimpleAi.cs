...
// based on Megagamefan100's code
...
public class SimpleAi : MonoBehaviour
{
    ...
    [SerializeField] bool on = true; //Is the AI active? this can be used to place pre-set enemies in you scene.

    [SerializeField] bool canFly = false; //Flying alters float behavior to ignore gravity. The enemy will fly up or down only to sustain floatHeight level.
    [SerializeField] float floatHeight = 0.0f; //If it can fly/hover, you need to let the AI know how high off the ground it should be.

    [SerializeField] bool runAway = false; //Is it the goal of this AI to keep it's distance? If so, it needs to have runaway active.
    [SerializeField] bool runTo = false; //Opposite to runaway, within a certain distance, the enemy will run toward the target.
    [SerializeField] float runDistance = 20.0f; //If the enemy should keep its distance, or charge in, at what point should they begin to run?
    [SerializeField] float runBufferDistance = 25.0f; //Smooth AI buffer. How far apart does AI/Target need to be before the run reason is ended.

    [SerializeField] int walkSpeed = 5;
    [SerializeField] int runSpeed = 8;
    [SerializeField] int randomSpeed = 4;
    [SerializeField] float rotationSpeed = 8.0f; //Rotation during movement modifier. If AI starts spinning at random, increase this value. (First check to make sure it's not due to visual radius limitations)
    [SerializeField] float visualRadius = 10.0f; //How close does the player need to be to be seen by the enemy? Set to 0 to remove this limitation.
    [SerializeField] float moveableRadius = 20.0f; //If the player is too far away, the AI will auto-matically shut down. Set to 0 to remove this limitation.

    [SerializeField] float attackRange = 10.0f; //How close does the enemy need to be in order to attack?
    [SerializeField] float attackTime = 0.50f; //How frequent or fast an enemy can attack (cool down time).

    [SerializeField] bool useWaypoints = false; //If true, the AI will make use of the waypoints assigned to it until over-ridden by another functionality.
    [SerializeField] bool reversePatrol = true; //if true, patrol units will walk forward and backward along their patrol.
    [SerializeField] Transform[] waypoints; //define a set path for them to follow.
    [SerializeField] bool pauseAtWaypoints = false; //if true, patrol units will pause momentarily at each waypoint as they reach them.
    [SerializeField] float pauseMin = 1.0f; //If pauseAtWaypoints is true, the unit will pause momentarily for minmum of this time.
    [SerializeField] float pauseMax = 3.0f; //If pauseAtWaypoints is true, the unit will pause momentarily formaximum of this time.
    [SerializeField] float huntingTimer = 5.0f; //Search for player timer in seconds. Minimum of 0.1
    [SerializeField] bool estimateElevation = false; //This implements a pause between raycasts for heights and guestimates the need to move up/down in height based on the previous raycast.
    [SerializeField] float estRayTimer = 1.0f; //The amount of time in seconds between raycasts for gravity and elevation checks.
    [SerializeField] bool requireTarget = true; //Waypoint ONLY functionality (still can fly and hover).
    [SerializeField] Transform target; //The target, or whatever the AI is looking for.
    ...
    void AIFunctionality()
    {
        ...
        else if (TargetIsInSight())
        {
            ...
            { //make sure they do not get too close to the target
                ...
                //RUN AWAY
                enemyCanAttack = false; //can't attack running...
    ...
    IEnumerator Attack()
    {
        ...
        if (!enemyIsAttacking)
        {
            ...
            while (enemyCanAttack)
            {
                ...
                // TODO :implement attack variables here
    ...
    void AnimateCharacter()
    {
        ...
        float velocityZ = characterController.velocity.z;
        ...
        if (isWalking)
        {
            ...
            if (!isAiming && !isAttacking && isIdle)
            {
                animator.SetTrigger("Walking 0");
                isIdle = false;
            }
        }
        else
